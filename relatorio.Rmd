...
habitacao_precaria_ra <- svy2018_pes %>%
  ...
  mutate(job=case_when(
    ...
    TRUE~"formal")) %>%
  # Agrupar por RA
 srvyr::group_by(RA_nome,job,domicilio) %>% 
...
kable(tabelahabprecaria, col.names = c("Região Administrativa", "domícílio improvisado", "domicílio rústico", "habitação precária")) %>%
