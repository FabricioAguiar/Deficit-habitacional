...
habitacao_precaria_ra <- svy2018_pes %>%
  ...
  mutate(job=case_when(
    ...
    TRUE~"formal")) %>%
  # Agrupar por RA
 srvyr::group_by(RA_nome,domicilio,genero,job,raca) %>% 
...
A<-habitacao_precaria_ra %>% 
  ...
  coord_flip()+
  ggtitle("Precariedade e suas clasificações - Região Administrativas")+
  labs( y = 'N° de domicílios', x = 'Região Administrativa',fill='Classificação')+
...
B<-habitacao_precaria_ra %>% 
  filter(!is.na(domicilio)) %>%
  ggplot(aes(fill =domicilio,x=fct_reorder(RA_nome,pct, .fun=sum,na.rm=FALSE,desc=TRUE), y=pct))+       geom_bar( stat = "identity")+
  coord_flip()+
  ggtitle("Precariedade e suas clasificações - Região Administrativas")+
  labs( y = 'N° de domicílios', x = 'Região Administrativa',fill='Classificação')+
  theme(legend.position="right")+theme(legend.text = element_text(size=14))
